...
import jittor
import os
from .cliff.cliff_resnet50 import CLIFF_resnet50
from .multi_ROI.mutilROI_resnet50 import MutilROI_resnet50
from .multi_ROI.mutil_resnet50_all import HMR_sim
from .metaHMR.metaHMR_resnet50 import MetaHMR_main, MetaHMR_aux
...
def build_model(smpl_mean_params, pretrained=True, backbone='resnet',
    ...
    if backbone == 'resnet':
        if model_name == 'cliff':
            model = CLIFF_resnet50(smpl_mean_params, **kwargs)
        elif model_name == 'hmr':
            model = hmr(smpl_mean_params, pretrained=pretrained, **kwargs)
            ...
            model = HMR_sim(Bottleneck, [3, 4, 6, 3], smpl_mean_params,
                            ...
                            wo_fuse=False,
                            wo_enc=False)
        elif model_name == "metaHMR_main":
            model = MetaHMR_main(Bottleneck, [3, 4, 6, 3], smpl_mean_params, **kwargs)
        elif model_name == "metaHMR_aux":
            model = MetaHMR_aux(Bottleneck, [3, 4, 6, 3], smpl_mean_params, **kwargs)

    from collections import OrderedDict
    if pretrained:
        print('Loading pretrained weights for ResNet backbone...')
        if model_name == 'metaHMR_aux':
            resnet_coco_ = jittor.load('data/best_ckpt/cliff_resnet_72.0_45.7.pkl')['model']
