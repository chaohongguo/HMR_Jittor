...
class SetCriterion(nn.Module):
    ...
    def loss_keypoint_3d(self, pred_keypoint_3d, gt_keypoint_3d, has_pose_3d, using_pseudo=False):
        """
        ...
        """
        pred_keypoint_3d = pred_keypoint_3d[:, 25:, :]  # [B,24,3]
        if using_pseudo:
            conf = jittor.ones_like(gt_keypoint_3d[:, :, -1].unsqueeze(-1)[:, 25:, :])
            gt_keypoint_3d = gt_keypoint_3d.clone()[:, 25:, :]  # [B,24,3]
        else:
    ...
    def loss_smpl(self, pred_rotmat, pred_betas, gt_pose, gt_betas, has_smpl, using_pseudo=False):
        """
    ...
    def execute(self, predict, gt, const, using_pseudo=False, is_eval=False, **kw):
        if is_eval:
            ...
            return loss_keypoint_2d_cliff, None

        using_pseudo = using_pseudo
        ...
        elif self.model_name == 'metaHMR':
            loss_keypoint_3d = self.loss_keypoint_3d(pred_keypoint_3d=predict['pred_output'].joints,
                                                     gt_keypoint_3d=gt['gt_joints'],
                                                     has_pose_3d=const['has_pose_3d'], using_pseudo=using_pseudo)
            loss = self.weight['loss_shape'] * loss_shape + \
                   self.weight['loss_keypoint_3d'] * loss_keypoint_3d + \
                   self.weight['loss_keypoint_2d'] * loss_keypoint_2d_cliff + \
                   self.weight['loss_pose'] * loss_regr_pose + \
