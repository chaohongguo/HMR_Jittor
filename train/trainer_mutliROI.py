...
from eval import run_evaluation_mutilROI
...
class Trainer(BaseTrainer):
    ...
    def train_step(self, input_batch, cur_step):
        ...
        crop_trans = jittor.concat([crop_trans.unsqueeze(1), crop_trans_extras], 1).view(batch_size * n_views, 2, 3)  # [b*v,2,3]
        ...
        # print("========================================================")
        # print(bbox_info)
        # np.savez('./jittor_model_input.npz',
        #          images=images.cpu().detach().numpy(),
        #          bbox_info=bbox_info.cpu().detach().numpy())
        start = time.time()
        pred_rotmat, pred_betas, pred_camera, xf_g = self.model(images, bbox_info)
        # end = time.time()
        # output_ = np.load('/home/lab345/mnt4T/__gcode_/HMR/mutil_roi/data/__data_look/torch_model_ouput.npz')
        # pred_rotmat = jittor.array(output_['pred_rotmat'], dtype=jittor.float32)
        # pred_betas = jittor.array(output_['pred_betas'], dtype=jittor.float32)
        # pred_camera = jittor.array(output_['pred_camera'], dtype=jittor.float32)
        # xf_g = jittor.array(output_['xf_g'], dtype=jittor.float32)
